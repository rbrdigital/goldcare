...
// Custom imaging icon (same as in ImagingOrderHeader)
...
interface ImagingOrderSummaryCardProps {
  order: ImagingOrder;
  orderNumber: number;
}

export function ImagingOrderSummaryCard({ order, orderNumber }: ImagingOrderSummaryCardProps) {
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-shrink-0">
          <div className="h-10 w-10 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center">
            <ImagingIcon className="h-5 w-5 text-primary" />
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {order.diagnosisCodes.length > 1 && (
            <div className="text-xs text-fg-muted mt-1">
              + {order.diagnosisCodes.length - 1} more {order.diagnosisCodes.length === 2 ? 'diagnosis' : 'diagnoses'}
            </div>
          )}
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-border">
          <div className="space-y-1">
            {order.studies.map((study, idx) => (
              <div key={study.id} className="text-xs">
                <span className="font-medium text-fg">{study.label}</span>
                {study.contrast && (
                  <span className="text-fg-muted ml-1">
                    ({study.contrast === 'yes' ? 'with contrast' : study.contrast === 'no' ? 'without contrast' : 'contrast unsure'})
                  </span>
                )}
              </div>
            ))}
