...
export function LabsImagingSideSheet({ isOpen, onClose }: LabsImagingSideSheetProps) {
  ...
  const getStatusChip = (status: string) => {
    ...
    return (
      <Badge 
        variant="outline" 
        className={cn("text-xs px-2 py-1", variants[status as keyof typeof variants])}
      >
        {status}
      </Badge>
    );
                    ...
                    <div className="mt-3 p-3 bg-muted rounded-md space-y-2">
                      <div>
                        ...
                        <div className="space-y-1">
                          {item.details.tests.map((test, testIndex) => (
                            <p key={testIndex} className="text-xs text-muted-foreground">• {test}</p>
                          ))}
                        </div>
                      </div>
