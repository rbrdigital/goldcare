...
type Props = {
  ...
  onInsert: () => void;
  onClose: () => void;
  onGenerate?: (prompt: string) => void; // optional callback
  className?: string;
};

export default function AIChipPanelCustomizable({
  text,
  onInsert,
  onClose,
  onGenerate,
  ...
  const handleGenerate = () => {
    if (onGenerate) onGenerate(prompt.trim());
    else window.alert(`GENERATE with: ${prompt.trim() || "(empty)"}`);
  };
  ...
  return (
    <div className="w-full">
      ...
      >
        {/* 12x12 star icon from AIChipPanel */}
        <svg 
        ...
        <span className="text-sm leading-5 min-w-0 whitespace-normal">{text}</span>
        ...
        <button
          ...
          onClick={onClose}
          className={cn(
            "text-sm text-fg-muted hover:underline focus-visible:outline-none ml-2",
            "focus-visible:ring-2 focus-visible:ring-primary rounded"
          )}
      ...
      {showCustom && (
        <div className="mt-2 rounded-md border border-border bg-surface p-3">
          <div className="text-sm font-medium mb-2">Refine this insight</div>
          <AutosizeTextarea
            minRows={2}
          ...
          <div className="mt-3 flex items-center gap-2 justify-end">
            <Button variant="subtle" onClick={() => setShowCustom(false)}>Cancel</Button>
            <Button className="bg-fg text-bg hover:opacity-90" onClick={handleGenerate}>
              GENERATE
            </Button>
          </div>
        </div>
