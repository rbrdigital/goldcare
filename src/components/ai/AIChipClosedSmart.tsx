...
export function AIChipClosedSmart({ text, onInsert, onGenerateInsert, useCustomizable = false, className }: AIChipClosedSmartProps) {
  ...
  return (
    <div 
      className={cn(
        "mt-2 w-full flex items-center justify-between rounded-full relative",
        "border border-gc-gold-500 bg-surface-muted px-3 py-2 text-[13px] text-fg",
        "transition-all duration-200",
        "hover:border-gc-gold-400",
        "focus-within:ring-2 focus-within:ring-gc-gold-400 focus-within:ring-opacity-50",
        "ai-chip-glow",
        className
      )}
      role="group"
      ...
      style={{
        boxShadow: '0 0 10px 2px var(--gc-gold-shadow)',
        '--hover-shadow': '0 0 14px 3px var(--gc-gold-shadow)'
    ...
    >
      <div className="min-w-0 flex items-center gap-2 overflow-hidden">
        <svg 
          ...
          viewBox="0 0 12 12" 
          fill="none"
          className="shrink-0 text-gc-gold-500"
          aria-hidden="true"
        >
        ...
        <strong className="shrink-0 text-gc-gold-500">GoldCare&nbsp;AI:</strong>
        <span 
          ref={previewRef}
